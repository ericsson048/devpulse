...
export default function CourseDetail() {
  ...
  const handleModuleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (editingModule) await api.updateModule(editingModule.id, moduleForm);
    else await api.createModule(+id!, moduleForm);
  ...
  const handleLessonSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (editingLesson) await api.updateLesson(editingLesson.id, lessonForm);
    else await api.createLesson(lessonModuleId, lessonForm);
  ...
  const modFooter = (
    <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
      ...
      />
      <Button
        label={editingModule ? "Update" : "Create"}
        icon="pi pi-check"
  ...
  const lesFooter = (
    <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
      ...
      />
      <Button
        label={editingLesson ? "Update" : "Create"}
        icon="pi pi-check"
  ...
  return (
    <div className="page fade-in">
      ...
      >
        <i className="pi pi-arrow-left" style={{ fontSize: 12 }} /> Back to
        Courses
      </Link>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {modules.map((mod) => (
          ...
          >
            ...
            >
              <i
                className={`pi ${expanded.has(mod.id) ? "pi-chevron-down" : "pi-chevron-right"}`}
              ...
              >
                <Button
                  icon={mod.is_published ? "pi pi-eye" : "pi pi-eye-slash"}
                  severity={mod.is_published ? "success" : "secondary"}
                  text
                  rounded
                  size="small"
              ...
              >
                {(lessons[mod.id] || []).map((l) => (
                  ...
                  >
                    ...
                    >
                      <i
                        className={`pi ${lessonIcon(l.lesson_type)}`}
                    ...
                    <div style={{ display: "flex", gap: 2 }}>
                      ...
                      <Button
                        icon={l.is_published ? "pi pi-eye" : "pi pi-eye-slash"}
                        severity={l.is_published ? "success" : "secondary"}
                        text
                        rounded
                        size="small"
